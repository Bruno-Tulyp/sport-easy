...
export const matchRoutes = factory.createApp().use(authMiddleware)
...
matchRoutes.get(
  ...
  async ({
    ...
    if (teamSlug) {
      const [belongs, data] = await userBelongsToTeam({ db, teamSlug, userId })
    ...
    const baseConfig = {
      columns: { location: true, startDate: true, opponent: true, slug: true },
